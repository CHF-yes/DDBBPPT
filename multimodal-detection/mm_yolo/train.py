...
def recipe_for(args):
    if getattr(args, "architecture", "") == "independent_p2_memory_v3":
        ...
        if getattr(args, "fusion_strategy", "legacy_residual_v2") == "v521_stage_a_v1":
            return "v521_explicit_coarse_global_local_thermal_ghost"
...
def main():
    ...
    if args.fusion_strategy in ("v52_stage_a_v1", "v521_stage_a_v1"):
        ...
        if data_path != _HERE / "data.py":
            raise RuntimeError(f"Stage A imported an unexpected dataset module: {data_path}")
        log(f"[Stage-A] dataset_module={data_path}")
    ...
    if args.ir_a0_cache:
        ...
        meta_base["ir_a0"] = {
            "version": ("v521_explicit_coarse_v1" if args.fusion_strategy == "v521_stage_a_v1"
                        else "v52_rotation_input_v1" if args.fusion_strategy == "v52_stage_a_v1"
                        else "v5.1.1-a0"),
    ...
    def _evaluate_for_stage(eval_model):
        ...
        v511_stage_a = args.fusion_strategy in (
            "v511_conditional_ir_v1", "v52_stage_a_v1", "v521_stage_a_v1")
        ...
        combined = {
            ...
            "selection": ("ir_map50_95_manual_stage_b_gate" if args.fusion_strategy in ("v52_stage_a_v1", "v521_stage_a_v1") else "ir_map50_95_with_rgb_floor" if v511_stage_a else
    ...
    if args.eval_initial and not args.resume:
        ...
        (out_dir / "val_best.json").write_text(json.dumps(
            ...
            encoding="utf-8")
        if args.fusion_strategy in ("v52_stage_a_v1", "v521_stage_a_v1") and "branches" in initial:
            # ep0 is the true V4.4 migration baseline.  Seed per-branch bests
            # before any optimizer step so a weaker later validation cannot
            # silently replace the actual RGB/IR/Depth starting checkpoints.
            with (out_dir / "branch_metrics.jsonl").open("a") as stream:
                stream.write(json.dumps({
                    "epoch": 0,
                    "branches": {k: float(v["map50_95"])
                                 for k, v in initial["branches"].items()},
                }) + "\n")
            for branch, metric in initial["branches"].items():
                ap = float(metric["map50_95"])
                branch_best[branch] = {"epoch": 0, "map50_95": ap}
                save_mm_checkpoint(
                    out_dir / "weights" / f"best_{branch}.pt", ema.ema,
                    epoch=0, best_map=ap,
                    ...
                    for name in names:
                        ...
                        if name == "ir" and getattr(model, "v52_ir_input", None) is not None:
                            ...
                            if args.fusion_strategy == "v521_stage_a_v1":
                                ...
                                branch_loss = branch_loss + (ir_affine_weight * affine_loss *
