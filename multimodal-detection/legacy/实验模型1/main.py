...
def _model_kwargs(h) -> dict:
    ...
    return dict(mega=h.mega, aux_heads=h.aux_heads,
                ...
                aux_edge_head=getattr(h, "aux_edge_head", False),
                edge_head_src=getattr(h, "edge_head_src", "ir"),
...
def cmd_train(args):
    ...
    if args.no_mega:
        ov_h3["mega"] = False
    if args.edge_head:
        ov_h3["aux_edge_head"] = True
    if args.no_edge_head:
        ov_h3["aux_edge_head"] = False
...
def cmd_predict(args):
    ...
    # P0-4: 优先按 checkpoint **自描述结构**重建（mega/门控/距离头等与训练严格一致），
    # 无 checkpoint 时才按当前 config 构建。
    if args.weights:
        model, _meta = MB.load_experiment1_checkpoint(
            args.weights, weights=h.pretrained_weights,
            nc=None if args.class_num is None else int(args.class_num), strict=True)
    else:
...
def main():
    ...
    ap.add_argument("--no-mega", action="store_true",
                    help="关闭 MEGA 边缘引导门控（隔离实验：其 Sobel 会把 mosaic 拼接缝当边缘）")
    ap.add_argument("--edge-head", action="store_true",
                    help="启用 Step4b 真边缘头（GT 框边界 BCE 监督，并把预测边缘喂进融合门控）")
    ap.add_argument("--no-edge-head", action="store_true", help="显式关闭真边缘头")
