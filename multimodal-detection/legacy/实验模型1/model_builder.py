...
class EdgeHead(nn.Module):
    """Step4b **真**边缘辅助头（有 GT、有 loss）——取代"固定 Sobel 门控"的 MEGA。

    与 `EdgeAttnGate` 的区别：
      * `EdgeAttnGate`：固定 Sobel 算边缘 → 只做乘法门控，**无监督、无参数学习边缘**；
      * `EdgeHead`：从**辅助流特征**预测边缘 logits（1 通道），用 **GT 框边界**（+可选
        depth 梯度软标签）做 BCE 监督 —— 梯度只回传所在辅助流。

    为什么挂在 IR/RGB 流而不是 depth 流：depth 是模型输入，挂在 depth 上只需"抄输入"
    就能把 loss 压到很低（距离头已踩过这个坑），学不到对检测有用的东西。
    """

    def __init__(self, c_in: int, mid: int = 64):
        super().__init__()
        self.conv = nn.Sequential(Conv(c_in, mid, 3), nn.Conv2d(mid, 1, 1))
        nn.init.constant_(self.conv[-1].bias, -2.0)     # 边缘稀疏 → 初始偏负
...
class FusionInjector(nn.Module):
    ...
    * hook 挂在**预训练层本身**上，Sequential 结构与 state_dict 键名保持不变
    ...
    def __init__(self, c_main: int, c_aux: int, n_mod: int = 2, mega: bool = False,
                 ...
                 alpha_max: float = 1.0, gate_edge: bool = True):
        super().__init__()
        self.n_mod = int(n_mod)
        self.per_modality_gate = bool(per_modality_gate)
        # gate_edge=False 用于加载 Step4b 之前的旧 checkpoint（门控输入少 1 个边缘通道，
        # 旧 S3 的门第一层是 c_main+c_aux+1 而不是 +2）。新训练一律 True。
        self.gate_edge = bool(gate_edge)
        ...
        # 门控输入通道：F + aux_m + mask(1) + 预测边缘(1，gate_edge=False 时无该通道)
        if self.per_modality_gate:
            c_gate_in = c_main + c_aux + 1 + (1 if self.gate_edge else 0)
            self.gate = nn.ModuleList(
                nn.Sequential(Conv(c_gate_in, hidden, 1), nn.Conv2d(hidden, 1, 1))
    ...
    def set_context(self, aux_list, edges=None, mask=None, edge=None):
        """aux_list: 每模态辅助特征 [aux_ir, aux_dep]；mask: Depth 有效性掩码 (B,1,H,W)；
        edge: **预测边缘图**（EdgeHead 的 sigmoid 输出，B,1,H',W'）——作为门的额外输入，
        让门控用"学出来的边界"而不是固定 Sobel。"""
        self._aux = aux_list
        self._edges = edges
        self._mask = mask
        self._edge = edge
    ...
    def hook(self, _module, _inputs, output):
        ...
        for m, aux in enumerate(self._aux):
            ...
            if self.per_modality_gate:
                ...
                # 预测边缘（可选）：形状对齐到特征分辨率
                if not self.gate_edge:
                    g = 1.0 + torch.tanh(self.gate[m](torch.cat([output, aux, mask], 1)))
                else:
                    if self._edge is None:
                        e = torch.zeros_like(p[:, :1])
                    else:
                        e = self._edge
                        if e.shape[-2:] != p.shape[-2:]:
                            e = F.interpolate(e, size=p.shape[-2:], mode="bilinear",
                                              align_corners=False)
                        e = e.to(dtype=p.dtype)
                    g = 1.0 + torch.tanh(self.gate[m](torch.cat([output, aux, mask, e], 1)))
                p = p * g
...
class Experiment1Model(nn.Module):
    ...
    def __init__(self, weights: str = "yolo11s.pt", nc: int = 12,
                 ...
                 aux_edge_head: bool = False,   # Step4b 真边缘头（GT 框边界监督）
                 edge_head_src: str = "ir",     # 边缘头挂在哪个流：ir（默认）| rgb→ir 同流
                 gate_edge: bool = True,        # 门控是否含"预测边缘"输入通道（Step4b 起为 True；
                                                # 加载 Step4b 之前的旧 checkpoint 需设 False）
        ...
        # 结构自描述：这些开关决定 state_dict 的键集合，必须随 checkpoint 一起保存，
        # 否则加载方按默认配置重建会出现 missing/unexpected（例如 --no-mega 训练的
        # checkpoint 缺 12 个 MEGA 张量，按 mega=True 重建就会静默错配）。
        self._struct = dict(
            weights=str(weights), nc=int(nc), aux_ch=tuple(int(c) for c in aux_ch),
            dropout_p=float(dropout_p), mega=bool(mega), aux_heads=bool(aux_heads),
            aux_depth_head=bool(aux_depth_head), dist_head_src=str(dist_head_src),
            aux_edge_head=bool(aux_edge_head), edge_head_src=str(edge_head_src),
            gate_edge=bool(gate_edge),
            per_modality_gate=bool(per_modality_gate), gamma_init=float(gamma_init),
        )
        ...
        inj_kw = dict(n_mod=2, hidden=32, gamma_init=float(gamma_init),
                      per_modality_gate=bool(per_modality_gate), gate_edge=bool(gate_edge))
        ...
        # ---- Step4b: 真边缘头（GT 框边界 BCE 监督；预测边缘同时喂进融合门控）----
        self.edge_enabled = bool(aux_edge_head)
        self.edge_head_src = str(edge_head_src)
        self._aux_edge_logits = None
        if self.edge_enabled:
            self.aux_ir_edge_head = EdgeHead(ca2)
    ...
    def structure_kwargs(self) -> dict:
        """返回重建本模型所需的**全部**结构开关（供 checkpoint 自描述与校验）。"""
    ...
    def forward(self, rgb: torch.Tensor, ir: torch.Tensor, depth: torch.Tensor):
        ...
        # Step4b: 真边缘头（P4 分辨率；GT 框边界）——预测边缘同时作为融合门控的输入
        edge_map = None
        if self.edge_enabled:
            logits_e = self.aux_ir_edge_head(f_ir[1])
            edge_map = torch.sigmoid(logits_e)
            if self.training:
                self._aux_edge_logits = logits_e

        dep_mask = depth[:, 1:2]                              # 深度有效性掩码（可信度先验）
        e_for_gate = edge_map if self.inject3.gate_edge else None
        self.inject3.set_context([f_ir[0], f_dep[0]], None, dep_mask, e_for_gate)
        self.inject4.set_context([f_ir[1], f_dep[1]], edges, dep_mask, e_for_gate)
        self.inject5.set_context([f_ir[2], f_dep[2]], edges, dep_mask, e_for_gate)
...
def load_experiment1_checkpoint(path, weights: str = None, strict: bool = True, **overrides):
    """**按 checkpoint 自描述的结构**重建实验模型1 并严格加载权重。

    * 结构开关（mega / per_modality_gate / aux_depth_head / dist_head_src / aux_ch /
      gamma_init / dropout_p / nc / aux_edge_head / edge_head_src / gate_edge）从
      `ckpt["structure"]` 读取，可用 overrides 覆盖
      （例如换预训练权重路径、或强制 nc）；
    * 默认 strict=True：任何 missing/unexpected 直接报错，避免"结构对不上却静默忽略"
      （历史教训：--no-mega 训练的 checkpoint 缺 12 个 MEGA 张量，按默认 mega=True
      重建会得到 missing=12，MEGA 停在恒等态，成绩被误读）。
    返回 (model, ckpt_meta)。
    """
    ck = torch.load(str(path), map_location="cpu", weights_only=False)
    if not isinstance(ck, dict) or "model_state" not in ck:
        raise KeyError(f"[load_exp1] {path} 不是本框架的 checkpoint（缺 model_state）")
    st = dict(ck.get("structure") or {})
    if not st:
        # 旧格式 checkpoint（本次改动之前训练的）没有 structure 字段：无法自动推断，
        # 只能用 config 默认值 + 调用方显式 overrides，并**明确警告**；加载仍严格校验。
        h = MC.EXPERIMENT1.hyper
        st = dict(weights=str(MC.resolve_pretrained_weights(h.pretrained_weights)),
                  nc=int(MC.CLASS_NUM), aux_ch=(32, 48, 64),
                  dropout_p=float(h.modal_dropout_p), mega=bool(h.mega),
                  aux_heads=bool(h.aux_heads), aux_depth_head=bool(h.aux_depth_head),
                  dist_head_src=str(getattr(h, "aux_dist_head_src", "ir")),
                  per_modality_gate=bool(h.per_modality_gate),
                  gamma_init=float(h.gamma_init))
        print(f"[load_exp1] ⚠ {Path(path).name} 是旧格式 checkpoint（无 structure 字段），"
              f"结构开关取 config + 显式覆盖：请确认与实际训练一致，否则会报 missing/unexpected。")
    st.pop("weights", None)                    # 预训练权重路径不作为结构键
    st.update(overrides)
    # nc 优先级：显式 override > checkpoint 元数据 > structure 记录 > 赛题默认
    if overrides.get("nc") is not None:
        nc_v = int(overrides["nc"])
    elif ck.get("class_num"):
        nc_v = int(ck["class_num"])
    else:
        nc_v = int(st.get("nc") or MC.CLASS_NUM)
    st["nc"] = nc_v
    cn = list(ck.get("class_names") or [])
    if cn and len(cn) == nc_v:
        st["class_names"] = cn
    elif cn and "class_names" not in st:
        # 旧格式 checkpoint 的 class_names 恒为赛题 12 类（与 nc 可能不符）→ 忽略，用默认名
        print(f"[load_exp1] 注：checkpoint 的 class_names({len(cn)}) 与 nc({nc_v}) 不一致，"
              f"忽略并改用默认类别名")
    if weights:
        st["weights"] = str(weights)
    model = Experiment1Model(**st)
    missing, unexpected = model.load_state_dict(ck["model_state"], strict=False)
    if strict and (missing or unexpected):
        raise RuntimeError(
            f"[load_exp1] 结构不匹配：missing={len(missing)} unexpected={len(unexpected)}\n"
            f"  missing 前几个: {list(missing)[:6]}\n"
            f"  unexpected 前几个: {list(unexpected)[:6]}\n"
            f"  checkpoint structure: { {k: v for k, v in (ck.get('structure') or {}).items() if k != 'weights'} }")
    meta = {k: ck.get(k) for k in ("epoch", "best_map", "cfg_key", "class_num",
                                   "model_type", "structure") if k in ck}
    print(f"[load_exp1] {path} 加载完成（结构自描述）: epoch={ck.get('epoch')} "
          f"best_map={ck.get('best_map')} missing={len(missing)} unexpected={len(unexpected)}")
    return model, meta
